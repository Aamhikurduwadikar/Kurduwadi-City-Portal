...
(function(){
  function compact(){
    ...
    const grid=document.getElementById('homePrideGrid');
    ...
    cards.forEach((card,i)=>{if(i>=2)card.hidden=true;});
    if(cards.length>2){
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    const start=()=>{const grid=document.getElementById('homePrideGrid');if(grid)obs.observe(grid,{childList:true,subtree:true});};
