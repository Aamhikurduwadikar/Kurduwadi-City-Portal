/* Fast homepage media loader: fetch only when the gallery is near the viewport. */
(function(){
  ...
  function watch(){const host=document.getElementById('homeGallery');if(!host)return;const io=new IntersectionObserver(es=>{if(es.some(e=>e.isIntersecting)){io.disconnect();run(host)}},{rootMargin:'350px'});io.observe(host);}
